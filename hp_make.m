...
% MRS processing
    ...
    case 'sp_proc_for_fslmrs'
        %% use as hp_make('sp_proc_for_fslmrs', id, case)
        % id - id of the precossed subject
        % sp_name - type of the processed spectra {'sham', 'ref', 'act'}
        ...
        nam = sprintf('sub_%02i', id);
        case_nam = varargin{2};
        sp_name = sprintf('%s_%s', nam, case_nam );
        sp = io_loadspec_sdat([mainStruct.meta.folder '\' nam '\sp\' sp_name '.SDAT'], 1);

        sp = op_alignAverages_fd(sp, 1.8, 2.2, 0.1, 'n');

        sp_av = op_averaging(sp);
        [sp_av, K, wppm, amp, alpha, ph, model]= op_removeWater(sp_av, [4.3 4.9], 6);
        [sp_av,phaseShift] = op_autophase(sp_av, 0 , 4.5, 0);

        sp.fids = sp.fids - repmat(model.fids, sp.sz(2), 1)';
        sp.specs = sp.specs - repmat(model.specs, 1, sp.sz(2));
        sp = op_addphase(sp, phaseShift);

        mrs_writeSDAT([mainStruct.meta.folder '\' nam '\sp\nifti\' sp_name 'pre.SDAT'], sp.fids);
        copyfile([mainStruct.meta.folder '\' nam '\sp\' sp_name '.SPAR'], [mainStruct.meta.folder '\' nam '\sp\nifti\' sp_name 'pre.SPAR']);
...
%                         mainStruct.(nam).proc.(condition).(sp_nam).HNAA = NAA_H;
    ...
    case 'save_mrstask2file'
        %use as hp_make('save_mrstask2file', id)
        %saves mainStruct.(nam).proc.tp_matrix to file
        mainStruct = hp_make('load');
        id = varargin{1};
        nam = sprintf('sub_%02i', id);
        
        mkdir([mainStruct.meta.folder mainStruct.(nam).folder '\sp\nifti'])        
        writematrix(mainStruct.(nam).proc.tp_matrix, [mainStruct.meta.folder mainStruct.(nam).folder '\sp\nifti\tp_matrix.csv']);
...
%                             sp_nam = sprintf('tp_%02i_sm_bc', ij);%temporally smoothed and BOLD-corrected data
    ...
    case 'VoxelMap'
        %use as hp_make('VoxelMap')
        mainStruct = hp_make('load');
        excludedIDs = [1:6, 8, 9, 11, 13,17, 20, 22, 26, 28, 32];
        k= 1;
        for i = 4:32
            nam = sprintf('sub_%02i', i);
            if ~ismember(i, excludedIDs)
                img = spm_vol([mainStruct.meta.folder '\' nam '\anat\derived\w' nam '_sp_mask_act.nii']);
                img_matrix{k} = spm_read_vols(img);
                k= k+1;
            end
        end
        mean_img = img_matrix{1};
        for i = 2:k-1
            mean_img = mean_img + img_matrix{i};
        end
        mean_img = mean_img/(k-1);
        mean_img = mean_img(end:-1:1, :, :);
        niftiwrite(mean_img,'E:\Alex\fMRS-heatPain\_meta\sp_mask_mean.nii');
    ...
    case 'MRS_QA'
        ...
        for i = 4:32
            ...
            for tp_i = 1:6
                ...
                qaTab(i, tp_i, 1) = mainStruct.(nam).proc.(cond).(tp_nam).SNR_NAA;
                qaTab(i, tp_i, 2) = mainStruct.(nam).proc.(cond).(tp_nam).NAAerr;
                qaTab(i, tp_i, 3) = mainStruct.(nam).proc.(cond).(tp_nam).Crerr;
                qaTab(i, tp_i, 4) = mainStruct.(nam).proc.(cond).(tp_nam).Glxerr;
                qaTab(i, tp_i, 5) = mainStruct.(nam).proc.(cond).(tp_nam).LWNAA;
            end
        end
...
%         end
   ...
   for i = 3:32
       nam = sprintf('sub_%02i', i);
       temp(i-2, 1) = mainStruct.(nam).proc.selected_temp;
   end
   hand = zeros([30, 1]); hand(7:9) = 1;
